## fetching data from cloud to local machine
...
def ask_oracle():
    engine = create_engine ('oracle://{user}:{password}@{sid}'.format(
	user='hr',
	password='Ayush1234',
	sid='ayush'
))
...
def main():
    ...
    print(df)
    engine_client = ask_oracle()
    our_table = ask_oracle()
    query2 = 'select * from {}'.format('friends')
    ...
    df.to_sql('friends', engine_client, if_exists='replace', index=False)
